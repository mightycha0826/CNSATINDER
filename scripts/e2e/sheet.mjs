...
// 학생 앱 시트(Sheet · ReportPicker · BackButton) — 대화방·편지 미리보기에서 열고 닫고 신고까지
...
try {
	...
	console.log('[편지]');
	await page.goto(U('/dev/letters?v=detail')); await page.waitForTimeout(1200);
	await page.locator('button.more').first().click(); await page.waitForTimeout(300);
	await page.screenshot({ path: `${SP}/sheet-${TAG}-letter-menu.png` });
	const items = await page.locator('.sheet .item').allInnerTexts();
	check('편지 메뉴', items.length >= 2, items.join(','));
	const rep = page.locator('.sheet .item', { hasText: '신고하기' });
	if (await rep.count()) {
		await rep.click(); await page.waitForTimeout(300);
		await page.screenshot({ path: `${SP}/sheet-${TAG}-letter-report.png` });
		check('편지 신고 시트: 제목 · 사유 7개', (await page.locator('.report h3').innerText()) === '무엇이 문제인가요?' && (await page.locator('.reason').count()) === 7);
		await page.locator('.sheet .item', { hasText: '취소' }).click(); await page.waitForTimeout(300);
		check('취소로 닫힘', (await page.locator('.scrim').count()) === 0);
	}
	await page.locator('button.back').click(); await page.waitForTimeout(800);
	check('편지 뒤로 → 피드', new URL(page.url()).pathname !== '/dev/letters' || !page.url().includes('v=detail'), page.url());
