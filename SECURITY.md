...
가장 중요하게 지키는 것은 **"누가 보냈는지"가 다른 학생에게 드러나지 않는 것**과
...
### 2. 데이터베이스 (Supabase)
- **모든 표에 RLS.** 학생이 직접 읽을 수 있는 건 내 방 · 내 메시지처럼 정책이 허락한 행뿐입니다. 이름 편지 표는 private 라 RPC 로만.
- **`private` 스키마는 API 에 노출되지 않습니다** (신고 · 증거 · 운영진 명단 · 명렬표 · 활동 기록 · AI 대기열).
- 쓰기는 거의 전부 **security definer 함수(RPC)** 로만 합니다. 학생이 부를 수 있는 함수 46개는 모두
...
- 채팅 상대의 계정 id 는 어떤 응답에도 나오지 않습니다. 방 안에서는 방 번호 · 자리(1/2) · 익명 이름만 씁니다.
  (예외: 이름 편지 검색 `dm_search` 는 편지를 보낼 대상을 가리키려고 **찾은 사람**의 계정 id 를 돌려줍니다. 이름 · 학년은 이미 보이는 정보라
  새로 드러나는 것은 없고, **보낸 사람**의 계정 · 이름은 받는 사람에게 어떤 응답 · 알림에도 나오지 않습니다.)
- 연타 · 도배 제한: 메시지(토큰 버킷 12개, 초당 1.5개) · 매칭 · 편지 · 댓글마다 DB 가 셉니다.
- 규칙 필터: 전화번호 · 학번 · "N학년 N반" · SNS 아이디 · 금칙어가 들어간 채팅 · 편지 · 댓글은 **DB 가 저장 전에 거절**합니다.

### 2-1. 이름 편지 (Phase 23)
학생을 이름으로 찾아 익명으로 편지를 보내는 기능이라 **괴롭힘 통로가 되지 않게** 여러 겹으로 막습니다.
| 무엇 | 어떻게 | 근거 |
|---|---|---|
| 사칭 막기 | 이름은 명렬표(학교 이메일 앞자리 = 학번)에서 붙고 학생이 못 고침. 명렬표에 없는 학생만 한 번 적고, 명렬표에 있는 이름은 못 적음 | `private.person` · `set_my_name` |
| 찾히지 않을 권리 | 설정 > 편지 받기 끄면 검색에도 안 나오고 새 편지도 안 옴(기본 켜짐). 검색은 두 글자 이상 · 10명까지 · 이름과 학년만 | `profiles.letters_open` · `dm_search` |
| 보낸 사람 익명 | 받는 사람에게는 편지마다 새로 뽑은 익명 이름만. 목록 · 편지 · 푸시 알림 어디에도 보낸 사람 계정 · 이름 없음 | `dm_inbox` · `dm_thread` · `dm_push_payload` |
| 도배 | 새 편지는 편지 한도(기본 하루 3통), 이어 쓰기는 댓글 한도. 상대가 답하기 전에는 3개까지 | `letter_bucket_take` · `private.dm_streak` |
...
| 11 | Advisor "RLS 켜짐 · 정책 없음"(22개 → Phase 23 뒤 26개) · "로그인 사용자가 definer 함수 실행 가능"(37개 → 46개) | 의도 | 유지 — 정책 없음 = 모두 거절(RPC 로만 접근). 학생용 함수는 위 2절대로 호출자를 확인 |

확인한 것(문제 없음): 운영자 쿠키 서명 · 만료 · 명단 재확인, 관리자 전용 화면의 역할 검사(화면 + DB),
방 · 편지 함수의 호출자 확인(비회원 → `not_member`), 알림 클릭 주소(같은 출처만), `{@html}` 없음, 커밋된 비밀 값 없음.

### 실DB 반영
- `supabase/schema.sql` Phase 23(이름 편지) — 2026-09-26 Supabase 커넥터로 실DB 에 적용. 실제 학생 계정 권한으로 이름 확인 · 검색 · 보내기 ·
  받은 편지 · 답장 · 끝내기 · 끝낸 뒤 다시 보내기 막힘, 표 직접 읽기 거부, 받는 사람 응답에 보낸 사람 id · 이름 없음을 확인(되돌린 트랜잭션 안에서).
  Advisor: 새 표 4개는 "정책 없음"(RPC 로만 — 의도), 학생 함수는 37 → 46개(dm_* 8개 · `set_my_name`), 새 경고 종류 없음.
