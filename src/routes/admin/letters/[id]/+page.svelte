...
<a class="back" href="/admin/letters">← 편지 신고 목록</a>

<header class="head">
	<div>
		<h1>{r.target_type === 'letter' ? '편지' : '댓글'} · {REASON_LABEL[r.reason] ?? r.reason}</h1>
...
<div class="grid">
	<section>
		<h2>신고 시점 사본 <span class="muted">{d.evidence.length}개</span></h2>
			...
			{#each d.evidence as e (e.ord)}
				<div class="m" class:target={e.kind === 'comment' || (e.kind === 'letter' && !r.comment_id)}>
					<div class="m-head">
						...
						<span class="muted">{e.alias ?? ''} · {fmt(e.sent_at)}</span>
					</div>
					<p class="body">{e.body}</p>
				</div>
			{/each}
		</div>
		<p class="hint">이름은 그 편지 안에서만 쓰는 임시 이름입니다. 다른 편지·채팅과 이어지지 않습니다.</p>
	</section>
	...
	<aside>
		<section class="card">
			...
			{#if live}
				<form method="POST" action="?/remove" use:enhance onsubmit={confirmRemove}>
					<button class="btn danger-btn">{r.comment_id ? '댓글 내리기' : '편지 내리기'}</button>
				</form>
			{/if}
		</section>
			...
			{#if d.history.length}
				<ul class="hist">
					{#each d.history as h (h.id)}
						<li>
							<a href="/admin/letters/{h.id}">{fmt(h.created_at)} · {REASON_LABEL[h.reason]}</a>
		...
		<section class="card">
			...
			{:else}
				<p class="hint">학교 이메일은 꼭 필요할 때만 확인하세요.<br />열람하면 누가 언제 봤는지 기록됩니다.</p>
