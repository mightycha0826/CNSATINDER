...
<section class="stats">
	<div class="stat" class:hot={data.stats.open_reports > 0}>
		<b class="num">{data.stats.open_reports}</b><span>미처리 신고</span>
	</div>
	<div class="stat"><b class="num">{data.stats.reviewing}</b><span>검토 중</span></div>
	<div class="stat"><b class="num">{data.stats.active_rooms}</b><span>진행 중 대화</span></div>
	<div class="stat"><b class="num">{data.stats.seeking_now}</b><span>상대 찾는 중</span></div>
	<div class="stat"><b class="num">{data.stats.rooms_24h}</b><span>24시간 대화</span></div>
	<div class="stat"><b class="num">{data.stats.restricted_users}</b><span>이용 제한 계정</span></div>
	<a class="stat state" class:off={!data.stats.is_open} href="/admin/settings">
...
{#if data.reports.length === 0}
	<p class="empty muted">
		{data.status === 'open' ? '처리할 신고가 없어요.' : '해당하는 신고가 없어요.'}
	</p>
{:else}
	<table>
		<thead>
		...
		<tbody>
			{#each data.reports as r (r.id)}
				<tr>
					...
					<td class="note"><a href="/admin/reports/{r.id}">{r.note || '(메모 없음)'}</a></td>
